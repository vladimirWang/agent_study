...
@tool(description="按指定城市查询天气, 消息以字符串形式返回")
def get_weather(city: str) -> str:
    return f"{city}天气晴天，气温26摄氏度，空气湿度50%, 南风1级，AQI21，最近6小时降雨概率极低"

@tool(description="获取用户所在城市名称，以纯字符串形式返回")
def get_user_location()-> str:
    return random.choice(["深圳", "上海", "济南"])
...
@tool(description="从外部系统获取指定用户在指定月份的使用记录，以纯字符串形式返回，如果未检索到返回空字符串")
def fetch_external_data(user_id: str, month: str) -> str:
    # return f"{user_id}, month: {month}"
